...
mira_perf_plot <- function(out, out_noise, df_combos_long, df_combos, reset_every = 20, 
  ...
  g2 <- ggplot(df_noise, aes(x = factor(as.numeric(noise_modifier)), y = benefit)) + 
    geom_boxplot() + 
    theme(text = element_text(size=16)) + 
  ...
  g5 <- ggplot(df_byaction, aes(x = factor(action_number), y=  fitness, 
                                color = condition)) + 
    geom_violin()
...
mira_policy_plot <- function(out) {
  ...
  g6 <- ggplot(clusters, aes(x = pc2, y = pc3, shape = condition, color = factor(cluster))) + 
    geom_point() + 
    theme(text = element_text(size = 16)) + 
    labs(x = paste0("PC1 (", importance[2]*100, "% of variance)"),
         y = paste0("PC3 (", importance[3]*100, "% of variance)"))
  
  g <- plot_grid(g4,g5,g6)
...
mira_traversal_plot<- function(out) {
  plot_graph <- function(g, legend = "none") {
    gg <- ggraph(g, layout = 'manual', x=x, y=y) + 
      geom_node_point(aes(color=val), size = 5.5, fill = "navy") + 
...
out_hm <- prep_mira_heatmap(total_eps = 500, update =FALSE)
...
ggsave(here("figs", "mira_traversal.tiff"), device = "tiff", units="in", width=7, height=5, dpi = 700)
ggsave(here("figs", "mira_traversal.png"), device = "png", units="in", width=7, height=5, dpi = 700)
