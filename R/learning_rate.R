...
lr_test <- function(episodes = 500, train_input = "fitness", iter = 3){
  ...
  lr_range = 10^seq(1,3,by= 0.2)/1e7
  
  for(i in 1:length(lr_range)) {
    for(j in 1:iter) {
      #run evodm
      out <- evol_deepmind(num_evols = 1, N = N, episodes = episodes, 
                           ...
                           mira = TRUE, 
                           learning_rate = lr_range[[i]]) #set it up to have no win conditions
      
      cleaned_out <- clean_out(out)
      agent <- out[[3]]
      out2 <- evol_deepmind(agent = agent, pre_trained= TRUE, episodes = 500)
      agent <- out2[[2]]
      ...
      save(
        ...
        file = here("data", "results", 
                    paste0("mira", i, "N", N, "D", num_drugs, "_lr", i, "_",
