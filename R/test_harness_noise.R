...
landscapes_test <- function(filename, episodes = 1000, train_input = "fitness", 
                            mira = FALSE, iter = 1, reset_every = 20, 
                            starting_position, noise_modifier =0) {
    ...
    if(mira) {
      for(j in 1:iter) {
          ...
          out <- evol_deepmind(num_evols = 1, N = N, episodes = episodes, 
                               ...
                               random_start = FALSE, 
                               noise = TRUE, noise_modifier = noise_modifier, 
                               num_drugs = num_drugs, 
                               sigma = 0.5, 
...
for(i in 1:length(noise_vec)) {
  landscapes_test(mira= TRUE, episodes = 500,
                  train_input = "fitness", iter = 10, reset_every = 20, 
                  starting_position = 0, noise_modifier = noise_vec[i])
}
