...
landscapes_test <- function(filename, episodes = 1000, train_input = "fitness", 
                            ...
                            starting_position, total_resistance = FALSE) {
    ...
    if(mira) {
      for(j in 1:iter) {
          ...
          out <- evol_deepmind(num_evols = 1, N = N, episodes = episodes, 
                               ...
                               total_resistance = total_resistance) #set it up to have no win conditions
...
landscapes_test(mira= TRUE, episodes = 500,
                ...
                starting_position = 0, total_resistance = TRUE)
